...
## I generate some data  
  ...
  lambda = 5
  beta0=-4; beta1=-2; beta2=0; beta3=-1; beta4=2;
  ...
  tau = 5 #FOLLOW-UP TIME OF INTEREST
  ...
  sourcenumber = 1 ## CENTER NUMBER
  
  ### Treatment model and Centre probability
  afit = glm2(A ~ X, family = binomial(link=logit), data = df)  ## HERE YOU SPECIFY TREATMENT MODEL
  df1 = df; df2 = df; df3 = df;
  df1$X=1; df2$X=2; df3$X=3;
  df$pred_obs_X1 = predict(afit, newdata = df1, type="response")
  df$pred_obs_X1 = ifelse(df$A==1, df$pred_obs_X1, 1-df$pred_obs_X1)
  df$pred_obs_X2 = predict(afit, newdata = df2, type="response")
  df$pred_obs_X2 = ifelse(df$A==1, df$pred_obs_X2, 1-df$pred_obs_X2)
  df$pred_obs_X3 = predict(afit, newdata = df3, type="response")
  df$pred_obs_X3 = ifelse(df$A==1, df$pred_obs_X3, 1-df$pred_obs_X3)
  
  centerfit =  multinom(X ~ L2*L1, data = df,trace=F) #### HERE YOU SPECIFY CENTER MODEL
  df$pred_cen_tmp = predict(centerfit, newdata = df, type="probs")
  df$pred_cen = df$pred_cen_tmp[,1]
  
  df$pred_obs = df$pred_obs_X1*df$pred_cen_tmp[,1] + 
    df$pred_obs_X2*df$pred_cen_tmp[,2] + 
    df$pred_obs_X3*df$pred_cen_tmp[,3]
  ...
  df$AL1 = df$A*df$L1
  df$L1L2 = df$L1*df$L2
  fit_init = coxph(Surv(T_tilde, delta) ~ A + L1 + L2 + AL1, data = df, method="breslow") ## HERE YOU SPECIFY YOUR HAZARDS MODEL FOR EVENT OF INTEREST
  ...
  df$z = as.vector(as.matrix(df %>% select(A, L1, L2, AL1)) %*% coef_init) ## ESTIMATED PARAMETRIC COMPONENT IN h(t|Z), i.e., exp(Z'beta)
  ...
  fit_c = coxph(Surv(T_tilde, C_stat) ~ A + L2, data = df, method="breslow") ## HERE YOU SPECIFY YOUR CENSORING HAZARDS MODEL
  coef_c = fit_c$coefficients
  df$z_c = as.matrix(df %>% select(A,L2)) %*% coef_c  ## ESTIMATED PARAMETRIC COMPONENT IN h_c(t|Z)
  ...
  impdat$z = as.vector(as.matrix(impdat %>% select(A, L1, L2, AL1)) %*% coef_init) # SAME COVARATES AS IN "FIT_INIT" ABOVE, BUT WITH A FIXED AT A=txt
  ...
  dH = NULL; start=NULL;  # here I calculate the baseline hazards, double check for R program baseline hazard
  ...
  myfunc = function(gamma)
  {
    ...
    for(i in 1:nrow(tmpdat))
    {
      ...
      tmpdat$weight_out = I(tmpdat$T_tilde>=tmptime & tmpdat$A==txt)*(tmpdat$pred_cen*tmpdat$surv_tau)/(tmpdat$pred_obs*tmpdat$surv_c*tmpdat$surv_t)
  ...
  myfuncup = function(newgamma)
  {
    ...
    for(i in 1:nrow(tmpdat))
    {
      ...
      tmpdat$weight_out = I(tmpdat$T_tilde>=tmptime & tmpdat$A==txt)*(tmpdat$pred_cen*tmpdat$surv_tau_up)/(tmpdat$pred_obs*tmpdat$surv_c*tmpdat$surv_t)
